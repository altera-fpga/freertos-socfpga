...
UNITY_INTERNAL_PTR actual, const UNITY_LINE_TYPE lineNumber, const char *msg )
{
...
void UnityAssertGreaterOrLessOrEqualNumber( const UNITY_INT threshold,
        const UNITY_INT actual, const UNITY_COMPARISON_T compare,
        const char *msg, const UNITY_LINE_TYPE lineNumber,
        const UNITY_DISPLAY_STYLE_T style )
{
    int failed = 0;
        ...
        const UNITY_FLAGS_T flags )
{
    ...
    while ( (elements > 0) && (elements--) )
    {
        ...
        {
            expected =
                    (UNITY_INTERNAL_PTR) ((const char*) expected + increment);
        }
...
void UnityAssertFloatsWithin( const UNITY_FLOAT delta,
        const UNITY_FLOAT expected, const UNITY_FLOAT actual, const char *msg,
        const UNITY_LINE_TYPE lineNumber )
{
    RETURN_IF_FAIL_OR_IGNORE;
...
void UnityAssertGreaterOrLessFloat( const UNITY_FLOAT threshold,
        const UNITY_FLOAT actual, const UNITY_COMPARISON_T compare,
        const char *msg, const UNITY_LINE_TYPE lineNumber )
{
    int failed;
        ...
        const UNITY_LINE_TYPE lineNumber, const UNITY_DISPLAY_STYLE_T style )
{
    ...
    if ( (style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT )
    {
        if ( actual > expected )
        {
            Unity.CurrentTestFailed = (((UNITY_UINT) actual
                    - (UNITY_UINT) expected) > delta);
        }
        else
        {
            Unity.CurrentTestFailed = (((UNITY_UINT) expected
        ...
        const UNITY_FLAGS_T flags )
{
    ...
    while ( (elements > 0) && (elements--) )
    {
        ...
        if ( (style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT )
        {
            ...
            {
                Unity.CurrentTestFailed = (((UNITY_UINT) actual_val
                        - (UNITY_UINT) expect_val) > delta);
            }
            else
            {
                Unity.CurrentTestFailed = (((UNITY_UINT) expect_val
...
void UnityAssertEqualStringArray( UNITY_INTERNAL_PTR expected,
        const char **actual, const UNITY_UINT32 num_elements, const char *msg,
        const UNITY_LINE_TYPE lineNumber, const UNITY_FLAGS_T flags )
{
    UNITY_UINT32 i = 0;
    UNITY_UINT32 j = 0;
...
UNITY_INTERNAL_PTR actual, const UNITY_UINT32 length,
        const UNITY_UINT32 num_elements, const char *msg,
        const UNITY_LINE_TYPE lineNumber, const UNITY_FLAGS_T flags )
{
...
{
    printf("Unity test function called");
...
void UnityBegin( const char *filename )
{
    ...
    UNITY_CLR_DETAILS();UNITY_OUTPUT_START();
}
...
int UnityEnd( void )
{
    ...
    UNITY_PRINT_EOL();
    UNITY_FLUSH_CALL();UNITY_OUTPUT_COMPLETE();
