...
void UnityTestRunner( unityfunction *setup, unityfunction *testBody,
        unityfunction *teardown, const char *printableName, const char *group,
...
int UnityGetCommandLineOptions( int argc, const char *argv[ ] )
{
    ...
    for ( i = 1; i < argc; )
    {
        if ( strcmp(argv[ i ], "-h") == 0 || strcmp(argv[ i ], "--help") == 0 )
        {
            ...
            UNITY_PRINT_EOL();
            UnityPrint(
                    "  -v          Verbose output: show all tests executed even if they pass");
            UNITY_PRINT_EOL();
            UnityPrint(
            ...
            UnityPrint(
                    "More information about Unity: https://www.throwtheswitch.org/unity");
            UNITY_PRINT_EOL();
#endif
        ...
        else if ( strcmp(argv[ i ], "-r") == 0 )
        {
            ...
            if ( i < argc )
            {
                if ( *(argv[ i ]) >= '0' && *(argv[ i ]) <= '9' )
                {
                    ...
                    while ( argv[ i ][ digit ] >= '0'
                            && argv[ i ][ digit ] <= '9' )
                    {
                        UnityFixture.RepeatCount *= 10;
                        UnityFixture.RepeatCount +=
                                (unsigned int) argv[ i ][ digit++ ] - '0';
                    }
                    i++;
                }
            }
