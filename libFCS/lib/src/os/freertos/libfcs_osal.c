        ...
        FCS_OSAL_SIZE *size)
{
    ...
    *data = fdt_getprop(fit, noffset, FIT_DATA_PROP,
            &len);
    if (!*data) {
        fcs_freertos_fit_get_debug(fit, noffset, FIT_DATA_PROP, len);
        *size = 0;
        return -1;
    }
...
FCS_OSAL_INT fcs_freertos_open_service_session(struct fcs_cmd_context *ctx)
{
    ...
    if (ret >= 0)
    {
        *ctx->error_code_addr = ret;
        if (ret == 0)
        {
            memcpy(ctx->open_session.suuid, uuid, FCS_OSAL_UUID_SIZE);
            *ctx->open_session.suuid_len = FCS_OSAL_UUID_SIZE;
        }
        else
        {
            memset(ctx->open_session.suuid, 0U, FCS_OSAL_UUID_SIZE);
...
FCS_OSAL_INT fcs_freertos_hkdf_request(struct fcs_cmd_context *ctx)
{
    ...
    input_data = (char*)pvPortMalloc(HKDF_MAX_SIZE);
    if (input_data == NULL)
    {
        return -ENOMEM;
    }
     ...
     */
    temp = input_data;
    memset(temp, 0, HKDF_MAX_SIZE);
    memcpy(temp, &ctx->hkdf_req.ikm_len, sizeof(ctx->hkdf_req.ikm_len));
    temp += sizeof(ctx->hkdf_req.ikm_len);
    memcpy(temp, ctx->hkdf_req.ikm, ctx->hkdf_req.ikm_len);
    temp += HKDF_INPUT_SIZE;
    memcpy(temp, &ctx->hkdf_req.info_len, sizeof(ctx->hkdf_req.info_len));
    temp += sizeof(ctx->hkdf_req.info_len);
    memcpy(temp, ctx->hkdf_req.info, ctx->hkdf_req.info_len);
    temp += HKDF_INPUT_SIZE;
    memcpy(temp, ctx->hkdf_req.output_key_obj,
...
FCS_OSAL_INT fcs_freertos_aes_crypt(struct fcs_cmd_context *ctx)
{
    if (ctx->aes.mode == FCS_AES_GCM)
    {
        *ctx->aes.op_len = ctx->aes.ip_len + 16;
    }
    else if (ctx->aes.mode == FCS_AES_GCM_GHASH)
    {
        *ctx->aes.op_len = 0;
    }
    else
...
FCS_OSAL_INT fcs_freertos_sdos_encrypt(struct fcs_cmd_context *ctx)
{
    memcpy(&ctx->sdos.own, ctx->sdos.src + SDOS_OWNER_ID_OFFSET,
            SDOS_OWNER_ID_SIZE);
 ...
 * @return 0 on success, negative value on error.
 */
FCS_OSAL_INT fcs_freertos_api_binding(struct libfcs_osal_intf *intf)
{
    if (intf == NULL) {
